//Au choix :
// __p1__ et __arg2__ sont des points {x:,y:}
// Ou alors __p1__ est un point et __arg2__ est un vecteur de type vecteur

var Bipoint = function(__p1__,__arg2__)
{
	//==========================
	...
	//==========================
		...
		if(__arg2__ instanceof Vecteur)//Si l'arg2 est un vecteur
		{
			__p2__={x:__p1__.x+__arg2__.dx(),y:__p1__.y+__arg2__.dy()}
			__vecteur__=__arg2__;
		}
		else//Sinon, c'est un autre point
		{
			__p2__=__arg2__
			__vecteur__=new Vecteur({dx:__p2__.x-__p1__.x,dy:__p2__.y-__p1__.y});
		}
	...
	//==========================
		...
		//Getter/Setter de la coordonnées sur x (en pixels) du point 1
		this.X1=function(X)
		{
			if(typeof(X)!='undefined')
			{
				this._p1.x=schema.X2x(X);
				this.updateDessin();
			}
			return schema.x2X(this._p1.x);
		}

		//Getter/Setter de la coordonnées sur y (en pixels) du point 1
		this.Y1=function(Y)
		{
			if(typeof(Y)!='undefined')
			{
				this._p1.y=schema.Y2y(Y);
				this.updateDessin();
			}
			return schema.y2Y(this._p1.y);
		}
		
		//Getter/Setter de la coordonnées sur x (en pixels) du point 2
		this.X2=function(X)
		{
			if(typeof(X)!='undefined')
			{
				this._p2.x=schema.X2x(X);
				this.updateDessin();
			}
			return schema.x2X(this._p2.x);
		}

		//Getter/Setter de la coordonnées sur y (en pixels) du point 2
		this.Y2=function(Y)
		{
			if(typeof(Y)!='undefined')
			{
				this._p2.y=schema.Y2y(Y);
				this.updateDessin();
			}
			return schema.y2Y(this._p2.y);
		}
		
		

		
		//norme de la fleche en pixel
		this.NORME=function()
		{
			return Math.sqrt((this.x1()-this.x2())*(this.x1()-this.x2())+(this.y1()-this.y2())*(this.y1()-this.y2()))*schema.echelle();
		}
		
		
		//Fonction qui renvoie la coordonnee sur x du vecteurs représenté par P1P2 (dans la base du schéma)
		this.dx=function(dx)
		{
			if(typeof(dx)!='undefined')
			{
				this._vecteur.dx(dx);
				this.updatePointFromVecteur();
				this.updateDessin();
			}
			return this._vecteur.dx();
		}
		//Fonction qui renvoie la coordonnee sur y du vecteurs représenté par P1P2 (dans la base du schéma)
		this.dy=function(dy)
		{
...
/*		//Fonction qui déplace p2 pour avoir une norme de 1 (dans la base du schéma)
		this.normalise=function()
		{
			var vx=this.x1()+this.u().x;
			var vy=this.y1()+this.u().y
			this.x2(vx);
			this.y2(vy);
		}*/
		
/*		//Fonction qui déplace p2 pour avoir une norme de 1 (en px)
		this.NORMALISE=function()
		{
			var vx=this.X1()+this.u().x;
			var vy=this.Y1()-this.u().y
